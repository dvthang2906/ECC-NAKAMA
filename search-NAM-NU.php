...
if($where == 'L' || $where == 'M'){
    $totalRecords = mysqli_query($conn, "SELECT * FROM `quan_ao` WHERE $where_price `id_quanao` LIKE '" . $where . "%'");
$totalRecords = $totalRecords->num_rows;
$totalPages = ceil($totalRecords / $item_per_page);
$products = mysqli_query($conn, "SELECT * FROM `quan_ao` WHERE $where_price `id_quanao` LIKE '" . $where . "%' ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
